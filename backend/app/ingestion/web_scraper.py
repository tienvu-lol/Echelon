...
class WebScraperAdapter(BaseSourceAdapter):
    ...
    def __init__(self, target_url: str, source_name: str, organization: str = "Unknown"):
        super().__init__()
        self.target_url = target_url
        self.source_name = source_name
        self.organization = organization
        self.client = httpx.Client(
            headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) EchelonBot/1.0'},
    ...
    def fetch_opportunities(self) -> Iterator[Opportunity]:
        ...
        keywords = ['intern', 'software', 'developer', 'engineer', 'data', 'analyst', 'co-op']
        ...
        for a_tag in soup.find_all('a', href=True):
            ...
            if any(k in text_lower for k in keywords) and ('intern' in text_lower or 'intern' in href_lower):
                
                # Context extraction: Try to find surrounding paragraph for a description
                parent = a_tag.find_parent(['div', 'li', 'td'])
                description = parent.get_text(separator=' ', strip=True) if parent else text
                ...
                opp_id = hashlib.md5(f"{self.organization}{text}{href}".encode('utf-8')).hexdigest()
