...
def evaluate_eligibility(
    profile: StudentProfile, opportunity: Opportunity
) -> tuple[str, list[str]]:
    ...
    if opportunity.class_years and profile.class_year:
        if profile.class_year not in opportunity.class_years:
            status = INELIGIBLE
            notes.append(
                f"Requires class year in {opportunity.class_years}, but student is {profile.class_year}."
            )
    elif opportunity.class_years and not profile.class_year:
        status = UNKNOWN
        notes.append(
            f"Requires class year in {opportunity.class_years}, but student's class year is unknown."
        )
    ...
    if opportunity.majors and profile.major:
        ...
        for req_major in opportunity.majors:
            if (
                req_major.lower() in student_major_lower
                or student_major_lower in req_major.lower()
            ):
                matched = True
                break
        if not matched:
            status = INELIGIBLE
            notes.append(
                f"Requires major in {opportunity.majors}, but student is {profile.major}."
            )
    ...
    if opportunity.school_restrictions:
        ...
        if not matched:
            status = INELIGIBLE
            notes.append(
                f"Opportunity is restricted to specific schools: {opportunity.school_restrictions}."
            )
