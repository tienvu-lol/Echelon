...
def ping_gemini() -> str:
    ...
    try:
        ...
        raise GeminiServiceError("Gemini provider request failed. Check logs for details.") from exc
...
def classify_opportunity(opportunity: Opportunity) -> list[CareerTrackAffinity]:
    ...
    taxonomy = [
        ...
        "cybersecurity", "systems_infrastructure", "cloud_devops",
        "embedded_firmware", "computer_hardware", "robotics_autonomy",
        "mobile_frontend", "backend_platform", "quant_fintech", "research"
    ]
...
def rerank_opportunities(profile: StudentProfile, preferences: CareerPreferences, candidates: list[Opportunity]) -> list[RecommendationResponseItem]:
    ...
    try:
        ...
        for item in parsed_items:
            if getattr(item, 'opportunity', None) and getattr(item.opportunity, 'id', None):
...
class AgentUpdate(PydanticBaseModel):
    assistant_reply: str
    updated_preferences: Optional[CareerPreferences] = None

def chat_agent(profile: StudentProfile, preferences: CareerPreferences, user_message: str) -> AgentUpdate:
    ...
    try:
        ...
        return AgentUpdate(assistant_reply="I'm having trouble connecting to my career brain right now. Try again later!")
